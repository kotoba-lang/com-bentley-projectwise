(ns bentley_projectwise.main-test
  "Contract + behavioral test for the bentley_projectwise-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "bentley_projectwise-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'bentley_projectwise.main-test)]
