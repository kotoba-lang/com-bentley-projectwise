(ns bentley_projectwise.main
  "Kotodama WASM entrypoint for the Bentley_projectwise clean-room actor (L4) — Clojure port.
  ...
  facts (`bentley_projectwise.<Entity>/<field>`); `*store*` is the in-memory
  materialization used by the contract test and by the WASM runtime before a
  live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bentley_projectwise")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bentley_projectwise.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bentley_projectwise-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bentley_projectwise-compat/ready)
